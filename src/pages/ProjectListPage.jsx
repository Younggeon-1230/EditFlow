  ...
  return (
    <main className="projects-page">
      <section className="projects-heading">
        ...
        <button className="primary-button projects-create-button" disabled={mutations.isMutating} onClick={() => openForm()} type="button"><span aria-hidden="true">+</span> 새 프로젝트 만들기</button>
      </section>
      <ProjectSearchBar value={searchTerm} onChange={setSearchTerm} />
      {mutations.error && !isFormOpen && <section className="reference-state error-state" role="alert"><p>{mutations.error}</p></section>}
      ...
      <section aria-labelledby="server-projects-title">
        ...
        {server.isLoading && server.projects.length === 0 ? (
          ...
          <div className="project-grid" aria-label="서버 프로젝트 목록">{serverProjects.map((project) => <ProjectCard disabled={mutations.isMutating} isDeleting={mutations.pendingMutation?.kind === 'delete' && mutations.pendingMutation.projectKind === 'server' && mutations.pendingMutation.projectId === project.id} key={project.id} project={project} projectKind="server" onEdit={(item) => openForm(item, 'server')} onDelete={(item) => handleDelete(item, 'server')} />)}</div>
        ) : (
      ...
      <section aria-labelledby="local-projects-title" className="legacy-projects-section">
        <div className="projects-section-heading"><div><p className="page-eyebrow">LOCAL / LEGACY</p><h2 id="local-projects-title">로컬에만 저장된 프로젝트</h2><p>상세 화면에서 프로젝트별로 서버 가져오기를 실행할 수 있습니다.</p></div></div>
        {localProjects.length > 0 ? <div className="project-grid" aria-label="로컬 프로젝트 목록">{localProjects.map((project) => <ProjectCard disabled={mutations.isMutating} isDeleting={mutations.pendingMutation?.kind === 'delete' && mutations.pendingMutation.projectKind === 'local' && mutations.pendingMutation.projectId === project.id} key={project.id} project={project} projectKind="local" onEdit={(item) => openForm(item, 'local')} onDelete={(item) => handleDelete(item, 'local')} />)}</div> : <div className="projects-empty compact"><p>{searchTerm ? '조건에 맞는 로컬 프로젝트가 없습니다.' : '로컬에만 남은 프로젝트가 없습니다.'}</p></div>}
      </section>

      {isFormOpen && <div className="form-overlay" role="presentation" onMouseDown={() => { if (!mutations.isMutating) setIsFormOpen(false) }}><section aria-labelledby="project-form-title" aria-modal="true" className="project-form-dialog" role="dialog" onMouseDown={(event) => event.stopPropagation()}><div className="project-form-heading"><div><p className="page-eyebrow">{editing ? 'EDIT PROJECT' : 'NEW PROJECT'}</p><h2 id="project-form-title">{editing ? '프로젝트 수정' : '새 프로젝트 만들기'}</h2></div><button aria-label="폼 닫기" className="icon-button" disabled={mutations.isMutating} onClick={() => setIsFormOpen(false)} type="button">×</button></div>{mutations.error && <div className="reference-state error-state" role="alert"><p>{mutations.error}</p></div>}<ProjectForm initialValue={editing?.project} isSubmitting={mutations.isMutating} onCancel={() => setIsFormOpen(false)} onSubmit={handleSubmit} /></section></div>}
    </main>
  )
}
